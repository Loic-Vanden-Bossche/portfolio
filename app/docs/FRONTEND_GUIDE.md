...
The site is one personal archive with two distinct in-page portfolio modes. Photography is always the initial mode; Development is the alternate mode.

- `PortfolioExperience` stores the active `PortfolioMode` locally. The modes are not separate routes because switching between them is part of the visual experience and should not trigger a page navigation.
